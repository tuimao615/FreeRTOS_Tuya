...
//全局变量避免
static WorkMode g_mode = WORK_MODE_CHUNJING;
static WorkType g_type = AUTO_CONTROL;
//臭氧是否打开状态
static SensorStatus cy_status = OFF;
...
void set_cy_status(SensorStatus status)
{
    cy_status = status;
}
...
uc8 jinghua_minutes[] = {10, 0, TIME_END}; //净化5分钟
...
void enter_jinghua_mode(void)			//启动净化模式
{
    ALL_turn_off();
    jinghua_on();
...
void run_jinghua_mode()
{
    ...
    //第1、6、10分钟启动臭氧工作一分钟
    if(jinghua_on_time == 1 || jinghua_on_time == 6 || jinghua_on_time == 10)
    {
        CY_turn(1);
        set_cy_status(ON);
    }
