...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_UV_STATE, DP_TYPE_BOOL},
  {DPID_FRAGRANCE_STATE, DP_TYPE_BOOL},
  {DPID_FRAGRANCE, DP_TYPE_BOOL},
  ...
  {DPID_CH2O, DP_TYPE_VALUE},
  {DPID_CO2, DP_TYPE_VALUE},
  {DPID_TEMP_TARGET, DP_TYPE_VALUE},
  {DPID_ALARM_BEEP, DP_TYPE_BOOL},
  {DPID_NTC, DP_TYPE_VALUE},
};
...
//    #error "请将MCU串口发送函数填入该函数,并删除该行"
  extern void WIFI_USART_Put_Char(unsigned char value);
  WIFI_USART_Put_Char(value);
  /*
      //Example:
...
//    #error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"
  ...
  mcu_dp_bool_update(DPID_AUTO,当前自动开关); //BOOL型数据上报;
  mcu_dp_bool_update(DPID_DRYING,当前烘干); //BOOL型数据上报;
  mcu_dp_value_update(DPID_COUNTDOWN,当前倒计时); //VALUE型数据上报;
  mcu_dp_value_update(DPID_COUNTDOWN_LEFT,当前剩余时间); //VALUE型数据上报;
  mcu_dp_fault_update(DPID_FAULT,当前故障告警); //故障型数据上报;
  mcu_dp_value_update(DPID_TEMP_CURRENT,当前当前温度); //VALUE型数据上报;
  mcu_dp_bool_update(DPID_UV_STATE,当前杀菌消毒状态); //BOOL型数据上报;
  mcu_dp_bool_update(DPID_FRAGRANCE_STATE,当前香薰状态); //BOOL型数据上报;
  mcu_dp_bool_update(DPID_FRAGRANCE,当前香薰); //BOOL型数据上报;
  mcu_dp_bool_update(DPID_START,当前启动); //BOOL型数据上报;
  mcu_dp_bool_update(DPID_PAUSE,当前暂停/继续); //BOOL型数据上报;
  mcu_dp_enum_update(DPID_WORK_STATE,当前工作状态); //枚举型数据上报;
  mcu_dp_value_update(DPID_HUMIDITY_CURRENT,当前当前湿度); //VALUE型数据上报;
  mcu_dp_value_update(DPID_TVOC,当前TVOC); //VALUE型数据上报;
  mcu_dp_value_update(DPID_CH2O,当前CH2O); //VALUE型数据上报;
  mcu_dp_value_update(DPID_CO2,当前CO2); //VALUE型数据上报;
  mcu_dp_value_update(DPID_TEMP_TARGET,当前目标温度); //VALUE型数据上报;
  mcu_dp_bool_update(DPID_ALARM_BEEP,当前报警提示音); //BOOL型数据上报;
  mcu_dp_value_update(DPID_NTC,当前热敏ntc); //VALUE型数据上报;
  */
  extern WorkMode get_work_mode(void);
  extern int DHT20_get_temperature(void);
  extern int DHT20_get_humidity(void);
  extern uint16_t JW01_get_TVOC(void);
  extern uint16_t JW01_get_CH2O(void);
  extern uint16_t JW01_get_CO2(void);
  mcu_dp_enum_update(DPID_MODE, get_work_mode()); //枚举型数据上报;
  mcu_dp_value_update(DPID_TEMP_CURRENT, DHT20_get_temperature()); //VALUE型数据上报;
  mcu_dp_value_update(DPID_HUMIDITY_CURRENT, DHT20_get_humidity()); //VALUE型数据上报;
  mcu_dp_value_update(DPID_TVOC,JW01_get_TVOC()); //VALUE型数据上报;
  mcu_dp_value_update(DPID_CH2O,JW01_get_CH2O()); //VALUE型数据上报;
  mcu_dp_value_update(DPID_CO2,JW01_get_CO2()); //VALUE型数据上报;
}
...
/*****************************************************************************
函数名称 : dp_download_auto_handle
功能描述 : 针对DPID_AUTO的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_auto_handle(const unsigned char value[], unsigned short length)
{
  //示例:当前DP类型为BOOL
  unsigned char ret;
  //0:off/1:on
  unsigned char auto_A;

  auto_A = mcu_get_dp_download_bool(value,length);
  if(auto_A == 0)
    {
      //bool off
    }
  ...
  //There should be a report after processing the DP
  ret = mcu_dp_bool_update(DPID_AUTO,auto_A);
  if(ret == SUCCESS)
    return SUCCESS;
  else
    return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_fragrance_handle
功能描述 : 针对DPID_FRAGRANCE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_fragrance_handle(const unsigned char value[], unsigned short length)
{
  //示例:当前DP类型为BOOL
  unsigned char ret;
  //0:off/1:on
  unsigned char fragrance;

  fragrance = mcu_get_dp_download_bool(value,length);
  if(fragrance == 0)
    {
      //bool off
    }
  ...
  //There should be a report after processing the DP
  ret = mcu_dp_bool_update(DPID_FRAGRANCE,fragrance);
  if(ret == SUCCESS)
    return SUCCESS;
  else
    return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_temp_target_handle
功能描述 : 针对DPID_TEMP_TARGET的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_temp_target_handle(const unsigned char value[], unsigned short length)
{
  //示例:当前DP类型为VALUE
  unsigned char ret;
  unsigned long temp_target;

  temp_target = mcu_get_dp_download_value(value,length);
  /*
  //VALUE type data processing
  ...
  */
  ...
  //There should be a report after processing the DP
  ret = mcu_dp_value_update(DPID_TEMP_TARGET,temp_target);
  if(ret == SUCCESS)
    return SUCCESS;
  else
    return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_alarm_beep_handle
功能描述 : 针对DPID_ALARM_BEEP的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_alarm_beep_handle(const unsigned char value[], unsigned short length)
{
  //示例:当前DP类型为BOOL
  unsigned char ret;
  //0:off/1:on
  unsigned char alarm_beep;

  alarm_beep = mcu_get_dp_download_bool(value,length);
  if(alarm_beep == 0)
    {
      //bool off
    }
  ...
  //There should be a report after processing the DP
  ret = mcu_dp_bool_update(DPID_ALARM_BEEP,alarm_beep);
  if(ret == SUCCESS)
    return SUCCESS;
  else
    return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
  ...
  ***********************************/
  ...
  switch(dpid)
    ...
    case DPID_AUTO:
      //自动开关处理函数
      ret = dp_download_auto_handle(value,length);
      break;
    case DPID_DRYING:
      //烘干处理函数
    ...
    case DPID_FRAGRANCE:
      //香薰处理函数
      ret = dp_download_fragrance_handle(value,length);
      break;
    case DPID_START:
      //启动处理函数
    ...
    case DPID_PAUSE:
      ...
      break;
    case DPID_TEMP_TARGET:
      //目标温度处理函数
      ret = dp_download_temp_target_handle(value,length);
      break;
    case DPID_ALARM_BEEP:
      //报警提示音处理函数
      ret = dp_download_alarm_beep_handle(value,length);
      break;
